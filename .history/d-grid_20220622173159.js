    ...
    for (let gridElement of gridElements) {
      ...
      for (let attribute of gridElement.attributes) {
        ...
        if (at.includes("gfbs-")) {
          ...
          } while (alreadyCreatedClassesNames.includes(newRandomClassName));
          let gfbStringed = `.${gfb}-${newRandomClassName}`;
          console.log(`{${gfbStringed}DIVISOR${attValue}`);
          let aCrClMessage;
          for (let aCrCl of alreadyCreatedClasses) {
            let aClass = aCrCl.split("DIVISOR")[0];
            let aAttVal = aCrCl.split("DIVISOR")[0];
            if (attValue === aAttVal) {
          ...
          newClasses.push(`{${gfbStringed}DIVISOR${attValue}`);
          ...
          if (gfbStringed.includes("{") && gfbStringed.includes("}")) {
            if (hasBP === true) {
              ...
              switch (befSplited[2]) {
                case "sm":
                  befsStringedSM += gfbStringed;
                  break;
                case "md":
                  befsStringedMD += gfbStringed;
                  break;
                case "lg":
                  befsStringedLG += gfbStringed;
                  break;
                case "xl":
                  befsStringedXl += gfbStringed;
                  break;
                case "xxl":
                  befsStringedXXL += gfbStringed;
                  break;
              }
            } else {
              befsStringed += gfbStringed + "/";
            }
          }
        }
      }
