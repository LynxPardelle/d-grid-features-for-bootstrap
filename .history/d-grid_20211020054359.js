  ...
  for (let attribute of gridElement.attributes) {
    ...
    if(at.includes(atr)){
      if(at.includes('-sm') || at.includes('-md') || at.includes('-lg') || at.includes('-xl') || at.includes('-xll')){
        switch(true){
          case at.includes('-sm') && !atr.includes('-sm') && !atr.includes('-md') && !atr.includes('-lg') && !atr.includes('-xl') && !atr.includes('-xxl'):
            bp = 576;
          break;
          case at.includes('-md') && !atr.includes('-md') && !atr.includes('-lg') && !atr.includes('-xl') && !atr.includes('-xxl'):
            bp = 768;
          break;
          case at.includes('-lg') && !atr.includes('-lg')  && !atr.includes('-xl') && !atr.includes('-xxl'):
            bp = 992;
          break;
          case at.includes('-xl') && !atr.includes('-xl') && !atr.includes('-xxl'):
            bp = 1200;
          break;
          case at.includes('-xxl') && !atr.includes('-xxl'):
            bp = 1400;
          break;
        };
      }
...
for (let gridElement of gridElements) {
  for (let attribute of gridElement.attributes) {
    ...
    if ( at.includes("gfbs-") &&
      ...
      (at.includes('-md') && wWidth >= 992 && wWidth < 120) ||
      (at.includes('-md') && wWidth >= 1200 && wWidth < 1400) ||
      (at.includes('-md') && wWidth >= 1400))
    ) {
      switch(true){
        /* Container */
